...
Page {
    ...
    tools: ToolBarLayout {
        ...
        ToolButton {
            ...
            anchors.right: (pfoIsLocal) ? parent.right : backButton.right
            anchors.rightMargin: (pfoIsLocal) ? 0 : -(parent.width/4 + parent.width/32)
            onClicked: {
                reseteditSheet();
    ...
    function storeEditPfo() {
        ...
        errorState.reason = qsTr("Save to local") + ": \"" + appWindow.pfoName + "\"" + qsTr("created.") + "\n";
